...
def xcache_photo_size(id, size, crop=False, quality=90, download=False, constrain=False):
    """
    """
    ...
    image.save(response, "JPEG", quality=quality) # wch: the pixels in the PIL image are converted to JPEG in response
    image.close()

    if photo.is_public_photo() and photo.is_public_photoset():
        file_name = photo.image_filename() # wch: 'fish-playing-serene-beautiful-lighting-pastel-warm-color-palette-ece69.jpeg'
        file_path = 'cached%s%s' % (request_path, file_name) # wch: 'cached/photos/344/200x200/crop/40/fish-playing-serene-beautiful-lighting-pastel-warm-color-palette-ece69.jpeg'
        default_storage.save(file_path, ContentFile(response.content)) # wch: writes JPEG to cache disk location
        full_file_path = "%s%s" % (settings.MEDIA_URL, file_path) # wch: '/media/cached/photos/344/200x200/crop/40/fish-playing-serene-beautiful-lighting-pastel-warm-color-palette-ece69.jpeg'
        cache.set(cache_key, full_file_path) # wch: 'tendenci.photo.344.200x200.cropped.40.' -> '/media/cached/photos/344/200x200/crop/40/fish-playing-serene-beautiful-lighting-pastel-warm-color-palette-ece69.jpeg'
